...
function makeExpectedArticle(users, article, comments=[]) {
  ...
  return {
    ...
    date_modified: article.date_modified.toISOString(),
    number_of_comments,
    author: {
      id: author.id,
      ...
      date_modified: author.date_modified.toISOString(),
  ...
  return expectedComments.map(comment => {
    ...
    return {
      ...
      user: {
        ...
        date_modified: commentUser.date_modified.toISOString(),
...
function seedUsers(db, users) {
  ...
  return db.into('rk_users').insert(preppedUsers)
    .then(() =>
      // update the auto sequence to stay in sync
      db.raw(
        `SELECT setval('rk_users_id_seq', ?)`,
        [users.length - 1],
      )
    )
}
